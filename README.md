...
A small, read-only service for a NAS. It fetches homework from Pronote on a
schedule, converts it to a stable JSON format, and serves the last successful
snapshot to an E-ink device.
...
Do not expose port 8080 directly to the internet: the web planner and its
`/planner.json` data source are intentionally unauthenticated. Keep the service
on the trusted LAN or access it through a VPN. The E-ink endpoints remain
